...
from rest_framework.permissions import IsAuthenticated, AllowAny  # NEW: Added AllowAny
...
from .serializers import (
    ...
    TeacherSerializer,
    ParentSerializer,
    UserRegistrationSerializer,  # NEW: Import registration serializer
    UserRegistrationWithTokenSerializer,  # NEW: Import registration with token serializer
)
...
# NEW: User Registration/Signup View
class UserRegistrationView(generics.CreateAPIView):
    """
    API View for user registration/signup.
    Allows new users to create accounts without authentication.
    """
    queryset = User.objects.all()
    serializer_class = UserRegistrationWithTokenSerializer
    permission_classes = [AllowAny]  # Allow unauthenticated access for signup
    
    def create(self, request, *args, **kwargs):
        """
        Create a new user account and return user data with token.
        """
    ...
    def handle_exception(self, exc):
        ...
        if hasattr(exc, 'detail') and isinstance(exc.detail, dict):
            ...
            if 'email' in exc.detail:
                if 'already exists' in str(exc.detail['email'][0]):
                    response.data = {
                        'error': 'An account with this email address already exists. Please try logging in instead.'
                    }
            elif 'password' in exc.detail:
                response.data = {
                    'error': 'Password does not meet security requirements. Please ensure it is at least 8 characters long.'
                }
