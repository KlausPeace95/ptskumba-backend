...
class GradeScale(models.Model):
    ...
    def get_rule(self, grade):
        ...
        rule = self.gradescalerule_set.filter(
            min_marks__lte=grade, max_marks__gte=grade
        ).first()
        if not rule:
...
class GradeScaleRule(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["min_marks", "max_marks", "grade_scale"]),
        ]

    def __str__(self):
        return f"{self.min_marks}-{self.max_marks} {self.letter_grade} {self.numeric_scale}"
    ...
    def save(self, *args, **kwargs):
        if self.min_marks >= self.max_marks:
            raise ValidationError("min_grade must be less than max_marks.")
